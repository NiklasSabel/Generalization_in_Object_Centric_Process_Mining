...
def main():
    ...
    def model_init(trial):
        ...
        if model_args.model_pretrained_checkpoint:
            my_model = ContrastiveClassifierModel(checkpoint_path=model_args.model_pretrained_checkpoint, len_tokenizer=len(train_dataset.tokenizer), model=model_args.tokenizer, **init_args)
            if model_args.grad_checkpoint:
                my_model.encoder.transformer._set_gradient_checkpointing(my_model.encoder.transformer.encoder, True)
            return my_model
        else:
            my_model = ContrastiveClassifierModel(len_tokenizer=len(train_dataset.tokenizer), model=model_args.tokenizer, **init_args)
            if model_args.grad_checkpoint:
                my_model.encoder.transformer._set_gradient_checkpointing(my_model.encoder.transformer.encoder, True)
            return my_model
    ...
    if training_args.do_train:
        ...
        train_dataset = ContrastiveClassificationDataset(train_dataset, dataset_type='train', size=data_args.train_size, tokenizer=model_args.tokenizer)
        if training_args.evaluation_strategy != 'no':
            validation_dataset = raw_datasets["validation"]
            validation_dataset = ContrastiveClassificationDataset(validation_dataset, dataset_type='validation', size=data_args.train_size, tokenizer=model_args.tokenizer)
        if training_args.load_best_model_at_end:
            test_dataset = raw_datasets["test"]
            test_dataset = ContrastiveClassificationDataset(test_dataset, dataset_type='test', size=data_args.train_size, tokenizer=model_args.tokenizer)
    ...
    callback = EarlyStoppingCallback(early_stopping_patience=15)
    ...
    if model_args.model_pretrained_checkpoint:
        model = ContrastiveClassifierModel(checkpoint_path=model_args.model_pretrained_checkpoint, len_tokenizer=len(train_dataset.tokenizer), model=model_args.tokenizer, **init_args)
        if model_args.grad_checkpoint:
            model.encoder.transformer._set_gradient_checkpointing(model.encoder.transformer.encoder, True)
    else:
        model = ContrastiveClassifierModel(len_tokenizer=len(train_dataset.tokenizer), model=model_args.tokenizer, **init_args)
        if model_args.grad_checkpoint:
            model.encoder.transformer._set_gradient_checkpointing(model.encoder.transformer.encoder, True)
